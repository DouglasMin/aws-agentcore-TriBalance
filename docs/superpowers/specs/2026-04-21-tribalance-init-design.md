...
| 10 | LangSmith 노출 | `LANGCHAIN_TRACING_V2=true` + `LANGCHAIN_PROJECT=tribalance` 후 대시보드에 run 확인 |
...
- Secrets Manager: `OPENAI_API_KEY`, `LANGCHAIN_API_KEY` 읽기
...
| 4 | LangSmith 트래픽 증가 → 비용 | `LANGCHAIN_TRACING_V2=false` 로 dev 중 토글 가능 |
